...
# all its incoming transactions (transfer, deposit, and withdraw). The bank has n accounts 
# numbered from 1 to n. The initial balance of each account is stored in a 0-indexed integer 
...
#         account numbered account1 to the account numbered account2. Return true if the 
...
class Bank(object):
    ...
    def transfer(self, account1, account2, money):
        if account1 <= self.n_accounts and account2 <= self.n_accounts and self.balance[account1 - 1] >= money:
    ...
    def withdraw(self, account, money):
        if account <= self.n_accounts and self.balance[account - 1] >= money:
            self.balance[account - 1] -= money
            return True
        return False   