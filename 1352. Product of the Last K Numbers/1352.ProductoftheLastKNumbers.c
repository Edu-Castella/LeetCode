...
ProductOfNumbers* productOfNumbersCreate() {
    ProductOfNumbers *productOfNumbers = calloc(1, sizeof(ProductOfNumbers));
    productOfNumbers->nums = calloc(40000, sizeof(int));
    productOfNumbers->numsSize = 0;
    return productOfNumbers;
}

void productOfNumbersAdd(ProductOfNumbers* obj, int num) {
    obj->nums[obj->numsSize++] = num;
}

int productOfNumbersGetProduct(ProductOfNumbers* obj, int k) {
    int res = 1;
    for(int i = (obj->numsSize - k); i < obj->numsSize; i++){
        res *= obj->nums[i];
    }
    return res;
}
